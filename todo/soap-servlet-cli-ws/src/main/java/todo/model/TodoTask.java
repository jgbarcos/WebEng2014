...
public class TodoTask {
	...
	/**
	 * TodoTask from TransferTask
	 */
	public TodoTask(TransferTask transfer){
		this.id = transfer.getId();
		this.title = transfer.getTitle();
		this.endDate = transfer.getEndDate();
		this.project = transfer.getProject();
		this.content = transfer.getContent();

		try {
			this.setPriority(TaskPriority.valueOf(transfer.getPriority().toUpperCase()));
		} catch (IllegalArgumentException | NullPointerException doNothing) {}
		try {
			this.setStatus(TaskStatus.valueOf(transfer.getStatus().toUpperCase()));
		} catch (IllegalArgumentException | NullPointerException doNothing) {}
	}
